...
{
    public class EtichetteUDCReport : BasicReport
    {
        public StampaEtichetteUdcModel modello;
        public List<UdcModel> udc;

        private int fontSizeTesto = 70;

        public EtichetteUDCReport(StampaEtichetteUdcModel modello, List<UdcModel> udc) : base(modello.ParametriGeneraliStampa)
        {
        ...
        private void _componiContenuto(IContainer container)
        {
            ...
            container.AlignCenter().AlignMiddle().Column(column =>
            {
                ...
                foreach (UdcModel item in udc)
                {
                    ...
                    Image img = b.Encode(BarcodeLib.TYPE.CODE128A, item.Cod_udc);
                    ...
                    column.Item().AlignCenter().Text(item.Cod_udc).FontSize(fontSizeTesto);
